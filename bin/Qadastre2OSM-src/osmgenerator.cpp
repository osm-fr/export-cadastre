...
void OSMGenerator::dumpOSM(const QString &fileName, QList<OSMPath> *paths, bool merge)
{
    ...
    writer.writeAttribute("minlat", QString::number(boundsLatLon[1].y(), 'f', 7));
    writer.writeAttribute("maxlat", QString::number(boundsLatLon[0].y(), 'f', 7));
    writer.writeAttribute("minlon", QString::number(boundsLatLon[0].x(), 'f', 7));
    writer.writeAttribute("maxlon", QString::number(boundsLatLon[1].x(), 'f', 7));
    ...
    foreach (const QPointF &node, tNodes) {
        ...
        writer.writeAttribute("lat", QString::number(node.y(), 'f', 7));
        writer.writeAttribute("lon", QString::number(node.x(), 'f', 7));
        i++;
    }
