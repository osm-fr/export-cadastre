...
// Polyfill for compatibility with older browsers:
...
class JoinPolygons {
    constructor(latLngs1, latLngs2) {
        ...
        let outer1_size = JoinPolygons.line_length(this.outer1_latLngs);
        let outer2_size = JoinPolygons.line_length(this.outer2_latLngs);
...
function betterPad(latLngBounds, meters, factor) {
    const cur_width_meters = latLngBounds.getSouthWest().distanceTo(latLngBounds.getSouthEast());
    const cur_height_meters = latLngBounds.getSouthWest().distanceTo(latLngBounds.getNorthWest());
    const cur_size_meters = latLngBounds.getSouthWest().distanceTo(latLngBounds.getNorthEast());
    const new_size_meters = (cur_size_meters + meters) * (1 + factor);
    const center = latLngBounds.getCenter();
    const new_width_deg = (latLngBounds.getEast()-center.lng) * new_size_meters / cur_width_meters;
    const new_height_deg = (latLngBounds.getNorth()-center.lat) * new_size_meters / cur_height_meters;
...
function connection_problem(err) {
    ...
    if ((err.name == 'ConnectionError') 
        || (err.name == "NetworkError") 
            || ((err.name == "TypeError") && err.message.startsWith("NetworkError")))
    {
...
function choose(choice, animate) {
    ...
    if ((cur_index >= 0) && (cur_index < cases.length)) {
        ...
        let url = "set.php?id=" + cases[cur_index].id + "&choice=" + choice + "&session=" + session;
        //console.log(url);
...
function go_back(animate) {
    ...
    if (cur_index >= 1) {
        ...
        let url = "set.php?id=" + cases[cur_index - 1].id + "&choice=back" + "&session=" + session;
        //console.log(url);
        fetch(url, { method: 'POST' }).then(r => check_ok(r)).catch(connection_problem);
