...
[ -d $1 ] || mkdir $1
cd $1
./data/project/cadastre.openstreetmap.fr//bin/Qadastre2OSM --list $1 > $1-liste.txt
sed -i "s/\([^ ]*\) - \(.*\) (.*)/\1 \2/" $1-liste.txt
while read l 
	do
		code=`echo $l|cut -d' ' -f1`
		ville=`echo $l|sed "s/[^ ]* //"`
		echo /data/project/cadastre.openstreetmap.fr//bin/Qadastre2OSM --download $1 $code "$ville"
		/data/project/cadastre.openstreetmap.fr//bin/Qadastre2OSM --download $1 $code "$ville"
